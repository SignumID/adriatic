...
}d.Granite.csrf={initialised:false,refreshToken:o,_clearToken:n};
...
v<w;
v++){this._handler[v][t].apply(d,u)
}this.then=function(y,x){(t==="resolve"?y:x).apply(d,u)
};
this._handler=null
}};
function f(u){var v=document.location.host;
var x=document.location.protocol;
var w="//"+v;
var t=x+w;
return(u===t||u.slice(0,t.length+1)===t+"/")||(u===w||u.slice(0,w.length+1)===w+"/")||!(/^(\/\/|http:|https:).*/.test(u))
}var q=":cq_csrf_token",h="CSRF-Token",m=Granite.HTTP.externalize("/libs/granite/csrf/token.json");
var l;
var s;
function p(t){if(d.console){console.warn("CSRF data not available;The data may be unavailable by design, such as during non-authenticated requests: "+t)
...
u.send();
return t
}function e(){var u=new XMLHttpRequest();
u.open("GET",m,false);
u.send();
try{return s=JSON.parse(u.responseText).token
}catch(t){p(t)
}}function n(){s=undefined;
o()
}function a(u){var v=u.getAttribute("action");
if(u.method.toUpperCase()==="GET"||(v&&!f(v))){return
}if(!s){e()
}if(!s){return
...
}function k(t){var u=function(w){var v=w.target;
...
XMLHttpRequest.prototype.open=function(v,t,u){if(f(t)&&v.toLowerCase()!=="get"){this._csrf=true;
this._async=u
...
return
}if(s){this.setRequestHeader(h,s);
...
l.then(function(v){t.setRequestHeader(h,v);
c.apply(t,u)
...
if(d.Node){var r=Node.prototype.appendChild;
...
k(t.contentWindow.document)
}}catch(u){if(t.src&&t.src.length&&f(t.src)){if(d.console){console.error("Unable to attach CSRF token to an iframe element on the same origin")
}}}}return t
}
...
var m=function(){if(e&&e!="0px"){$CQ("#"+d).css("height","")
}};
...
CQ_Analytics.crmSegment=new function(){var b=function(h,e,f){var d=CQ_Analytics.ClientContextMgr.getRegisteredStore("crmDataStore");
if(d){var g=d.getCustomer(h,f),c;
switch(e){case"COOKIE_EXISTS":c=g.exists;
break;
case"COOKIE_NOT_EXIST":c=!g.exists;
break;
case"CUSTOMER_BELONGS_TO":c=g.crmData&&a(g.crmData.segments,f.crmSegments);
break;
case"CUSTOMER_NOT_BELONG_TO":c=g.crmData&&!a(g.crmData.segments,f.crmSegments);
break;
default:c=false
}return c
}return false
};
var a=function(e,d){var c=false;
if(e&&d){[].forEach.call(e,function(f){if(d.indexOf(f)!==-1){c=true
}})
}return c
};
return{resolve:b}
};
(function(a,b){b(a).ready(function(){var d={childList:true},c=a.getElementsByClassName("target");
[].forEach.call(c,function(f){var e=new MutationObserver(function(){var g;
if(window.parent.Granite.author){g=window.parent.$&&window.parent.$(window.parent.document)
}else{g=parent.$&&parent.$(a)
}if(g){g.trigger("vml.initialize")
}});
e.observe(f,d)
})
})
}(document,$CQ));
if(CQ_Analytics.ClientContextMgr){CQ_Analytics.ClientContextMgr.addListener("storeregister",function(b,a){if("crmDataStore"===a.getName()){if(this.initTimer){window.clearTimeout(this.initTimer);
this.initTimer=null
}this.fireEvent("storesinitialize");
...
}if(!CQ_Analytics.AnalyticsDataStore){CQ_Analytics.AnalyticsDataStore=function(){};
CQ_Analytics.AnalyticsDataStore.prototype=new CQ_Analytics.JSONStore();
CQ_Analytics.AnalyticsDataStore.prototype.STOREKEY="ANALYTICS_DATA_STORE";
CQ_Analytics.AnalyticsDataStore.prototype.STORENAME="analyticsDataStore"
}if(!CQ_Analytics.SessionStorageStore){CQ_Analytics.SessionStorageStore=function(){};
CQ_Analytics.SessionStorageStore.prototype=new CQ_Analytics.SessionStore();
CQ_Analytics.SessionStorageStore.prototype.STOREKEY="SESSION_STORAGE_STORE";
CQ_Analytics.SessionStorageStore.prototype.STORENAME="sessionStorageStore";
CQ_Analytics.SessionStorageStore.prototype.init=function(){this.data=sessionStorage[this.STOREKEY]?JSON.parse(sessionStorage[this.STOREKEY]):{};
if(!this.data){this.data={};
for(var a in this.initProperty){this.data[a]=this.initProperty[a]
}this.persist()
}this.initialized=true;
...
CQ_Analytics.SessionStorageStore.prototype.setProperty=function(a,b){if(this.data==null){this.init()
}this.data[a]=b;
this.persist();
this.fireEvent("update",a)
};
CQ_Analytics.SessionStorageStore.prototype.setProperties=function(b){if(this.data==null){this.init()
}var c=[];
for(var a in b){if(b.hasOwnProperty(a)){c.push(a);
this.data[a]=b[a]
}}if(c.length>0){this.persist();
this.fireEvent("update",c)
}};
CQ_Analytics.SessionStorageStore.prototype.removeProperty=function(a){if(this.data==null){this.init()
}if(this.data[a]){delete this.data[a]
}this.persist();
this.fireEvent("update",a)
};
CQ_Analytics.SessionStorageStore.prototype.clear=function(){this.data=null;
this.persist()
};
CQ_Analytics.SessionStorageStore.prototype.reset=function(){if(this.data!=null){this.data=null;
this.persist();
this.fireEvent("update")
}};
CQ_Analytics.SessionStorageStore.prototype.persist=function(){sessionStorage[this.STOREKEY]=JSON.stringify(this.data)
}
}if(!CQ_Analytics.CRMDataStore){CQ_Analytics.CRMDataStore=function(){this.apiEndpoint=null;
this.queryParameters=null
};
CQ_Analytics.CRMDataStore.prototype=new CQ_Analytics.SessionStorageStore();
CQ_Analytics.CRMDataStore.prototype.STOREKEY="CRM_DATA_STORE";
CQ_Analytics.CRMDataStore.prototype.STORENAME="crmDataStore";
CQ_Analytics.CRMDataStore.prototype.getCustomer=function(f,e){var h=this,d=h.getProperty(f,true);
if(!d){var j=CQ_Analytics.Cookie.read(f);
h.setProperty(f,{exists:j!=null,digitalId:j});
d=h.getProperty(f,true)
}if(d.digitalId&&!d.crmData&&e.apiEndpoint){var a=typeof e.queryParameters!=="undefined"?e.queryParameters:"";
if(a===this.queryParameters&&e.apiEndpoint===this.apiEndpoint){return
}this.queryParameters=a;
this.apiEndpoint=e.apiEndpoint;
var c={digitalId:d.digitalId};
try{var g=JSON.parse(document.getElementById("gem-settings").innerText);
a.replace("?","").replace("$language",g.language).replace("$institute",g.instituteNumber).split("&").forEach(function(l){var k=l.split("=");
c[k[0]]=k[1]
})
}catch(b){console.warn("Missing gem-settings!")
}$CQ.ajax({url:e.apiEndpoint,type:"GET",data:c,success:function(k){if(Array.isArray(k)&&k.length){d.crmData=k[0];
h.setProperty(f,d)
}}})
}return d
}
